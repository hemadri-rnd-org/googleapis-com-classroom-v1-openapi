...
func Classroom_courses_coursework_studentsubmissions_getHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateClassroom_courses_coursework_studentsubmissions_getTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v1_courses_courseId_courseWork_courseWorkId_studentSubmissions_id",
		mcp.WithDescription("Returns a student submission. * `PERMISSION_DENIED` if the requesting user is not permitted to access the requested course, course work, or student submission or for access errors. * `INVALID_ARGUMENT` if the request is malformed. * `NOT_FOUND` if the requested course, course work, or student submission does not exist."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Classroom_courses_coursework_studentsubmissions_getHandler(cfg),
	}
}
