...
func Classroom_courses_students_listHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var result models.ListStudentsResponse
...
func CreateClassroom_courses_students_listTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v1_courses_courseId_students",
		mcp.WithDescription("Returns a list of students of this course that the requester is permitted to view. This method returns the following error codes: * `NOT_FOUND` if the course does not exist. * `PERMISSION_DENIED` for access errors."),
		...
		mcp.WithNumber("pageSize", mcp.Description("Maximum number of items to return. The default is 30 if unspecified or `0`. The server may return fewer than the specified number of results.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Classroom_courses_students_listHandler(cfg),
	}
}
