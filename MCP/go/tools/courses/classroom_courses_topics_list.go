...
func Classroom_courses_topics_listHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v1/courses/%s/topics%s", cfg.BaseURL, courseId, queryString)
		...
		var result models.ListTopicResponse
...
func CreateClassroom_courses_topics_listTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v1_courses_courseId_topics",
		mcp.WithDescription("Returns the list of topics that the requester is permitted to view. This method returns the following error codes: * `PERMISSION_DENIED` if the requesting user is not permitted to access the requested course or for access errors. * `INVALID_ARGUMENT` if the request is malformed. * `NOT_FOUND` if the requested course does not exist."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Classroom_courses_topics_listHandler(cfg),
	}
}
