...
func Classroom_courses_topics_deleteHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateClassroom_courses_topics_deleteTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_v1_courses_courseId_topics_id",
		...
		mcp.WithString("id", mcp.Required(), mcp.Description("Identifier of the topic to delete.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Classroom_courses_topics_deleteHandler(cfg),
	}
}
