...
func Classroom_courses_teachers_listHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var result models.ListTeachersResponse
...
func CreateClassroom_courses_teachers_listTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v1_courses_courseId_teachers",
		mcp.WithDescription("Returns a list of teachers of this course that the requester is permitted to view. This method returns the following error codes: * `NOT_FOUND` if the course does not exist. * `PERMISSION_DENIED` for access errors."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Classroom_courses_teachers_listHandler(cfg),
	}
}
