...
func Classroom_courses_coursework_studentsubmissions_turninHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.TurnInStudentSubmissionRequest
		...
		url := fmt.Sprintf("%s/v1/courses/%s/courseWork/%s/studentSubmissions/%s:turnIn%s", cfg.BaseURL, courseId, courseWorkId, id, queryString)
...
func CreateClassroom_courses_coursework_studentsubmissions_turninTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v1_courses_courseId_courseWork_courseWorkId_studentSubmissions_id_turnIn",
		mcp.WithDescription("Turns in a student submission. Turning in a student submission transfers ownership of attached Drive files to the teacher and may also update the submission state. This may only be called by the student that owns the specified student submission. This request must be made by the Developer Console project of the [OAuth client ID](https://support.google.com/cloud/answer/6158849) used to create the corresponding course work item. This method returns the following error codes: * `PERMISSION_DENIED` if the requesting user is not permitted to access the requested course or course work, turn in the requested student submission, or for access errors. * `INVALID_ARGUMENT` if the request is malformed. * `NOT_FOUND` if the requested course, course work, or student submission does not exist."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Classroom_courses_coursework_studentsubmissions_turninHandler(cfg),
	}
}
