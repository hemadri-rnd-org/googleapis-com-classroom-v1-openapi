...
func Classroom_courses_updateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateClassroom_courses_updateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_v1_courses_id",
		mcp.WithDescription("Updates a course. This method returns the following error codes: * `PERMISSION_DENIED` if the requesting user is not permitted to modify the requested course or for access errors. * `NOT_FOUND` if no course exists with the requested ID. * `FAILED_PRECONDITION` for the following request errors: * CourseNotModifiable"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Classroom_courses_updateHandler(cfg),
	}
}
