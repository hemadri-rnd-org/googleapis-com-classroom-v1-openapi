...
func Classroom_courses_coursework_studentsubmissions_listHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["late"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("late=%v", val))
		}
		...
		if val, ok := args["states"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("states=%v", val))
		}
		...
		url := fmt.Sprintf("%s/v1/courses/%s/courseWork/%s/studentSubmissions%s", cfg.BaseURL, courseId, courseWorkId, queryString)
		...
		var result models.ListStudentSubmissionsResponse
...
func CreateClassroom_courses_coursework_studentsubmissions_listTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v1_courses_courseId_courseWork_courseWorkId_studentSubmissions",
		mcp.WithDescription("Returns a list of student submissions that the requester is permitted to view, factoring in the OAuth scopes of the request. `-` may be specified as the `course_work_id` to include student submissions for multiple course work items. Course students may only view their own work. Course teachers and domain administrators may view all student submissions. This method returns the following error codes: * `PERMISSION_DENIED` if the requesting user is not permitted to access the requested course or course work, or for access errors. * `INVALID_ARGUMENT` if the request is malformed. * `NOT_FOUND` if the requested course does not exist."),
		...
		mcp.WithString("courseWorkId", mcp.Required(), mcp.Description("Identifier of the student work to request. This may be set to the string literal `\"-\"` to request student work for all course work in the specified course.")),
		mcp.WithString("late", mcp.Description("Requested lateness value. If specified, returned student submissions are restricted by the requested value. If unspecified, submissions are returned regardless of `late` value.")),
		...
		mcp.WithArray("states", mcp.Description("Requested submission states. If specified, returned student submissions match one of the specified submission states.")),
		mcp.WithString("userId", mcp.Description("Optional argument to restrict returned student work to those owned by the student with the specified identifier. The identifier can be one of the following: * the numeric identifier for the user * the email address of the user * the string literal `\"me\"`, indicating the requesting user")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Classroom_courses_coursework_studentsubmissions_listHandler(cfg),
	}
}
