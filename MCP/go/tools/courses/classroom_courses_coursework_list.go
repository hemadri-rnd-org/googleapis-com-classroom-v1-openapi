...
func Classroom_courses_coursework_listHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["courseWorkStates"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("courseWorkStates=%v", val))
		}
		...
		var result models.ListCourseWorkResponse
...
func CreateClassroom_courses_coursework_listTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v1_courses_courseId_courseWork",
		mcp.WithDescription("Returns a list of course work that the requester is permitted to view. Course students may only view `PUBLISHED` course work. Course teachers and domain administrators may view all course work. This method returns the following error codes: * `PERMISSION_DENIED` if the requesting user is not permitted to access the requested course or for access errors. * `INVALID_ARGUMENT` if the request is malformed. * `NOT_FOUND` if the requested course does not exist."),
		...
		mcp.WithArray("courseWorkStates", mcp.Description("Restriction on the work status to return. Only courseWork that matches is returned. If unspecified, items with a work status of `PUBLISHED` is returned.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Classroom_courses_coursework_listHandler(cfg),
	}
}
