...
func Classroom_courses_aliases_listHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v1/courses/%s/aliases%s", cfg.BaseURL, courseId, queryString)
		...
		var result models.ListCourseAliasesResponse
...
func CreateClassroom_courses_aliases_listTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v1_courses_courseId_aliases",
		mcp.WithDescription("Returns a list of aliases for a course. This method returns the following error codes: * `PERMISSION_DENIED` if the requesting user is not permitted to access the course or for access errors. * `NOT_FOUND` if the course does not exist."),
		mcp.WithString("courseId", mcp.Required(), mcp.Description("The identifier of the course. This identifier can be either the Classroom-assigned identifier or an alias.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Classroom_courses_aliases_listHandler(cfg),
	}
}
