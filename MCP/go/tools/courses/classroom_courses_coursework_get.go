...
func Classroom_courses_coursework_getHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateClassroom_courses_coursework_getTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v1_courses_courseId_courseWork_id",
		mcp.WithDescription("Returns course work. This method returns the following error codes: * `PERMISSION_DENIED` if the requesting user is not permitted to access the requested course or course work, or for access errors. * `INVALID_ARGUMENT` if the request is malformed. * `NOT_FOUND` if the requested course or course work does not exist."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Classroom_courses_coursework_getHandler(cfg),
	}
}
