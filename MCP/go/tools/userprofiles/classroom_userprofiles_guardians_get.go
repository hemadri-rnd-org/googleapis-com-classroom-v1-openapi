...
func Classroom_userprofiles_guardians_getHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateClassroom_userprofiles_guardians_getTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v1_userProfiles_studentId_guardians_guardianId",
		mcp.WithDescription("Returns a specific guardian. This method returns the following error codes: * `PERMISSION_DENIED` if no user that matches the provided `student_id` is visible to the requesting user, if the requesting user is not permitted to view guardian information for the student identified by the `student_id`, if guardians are not enabled for the domain in question, or for other access errors. * `INVALID_ARGUMENT` if a `student_id` is specified, but its format cannot be recognized (it is not an email address, nor a `student_id` from the API, nor the literal string `me`). * `NOT_FOUND` if the requesting user is permitted to view guardians for the requested `student_id`, but no `Guardian` record exists for that student that matches the provided `guardian_id`."),
		mcp.WithString("studentId", mcp.Required(), mcp.Description("The student whose guardian is being requested. One of the following: * the numeric identifier for the user * the email address of the user * the string literal `\"me\"`, indicating the requesting user")),
		mcp.WithString("guardianId", mcp.Required(), mcp.Description("The `id` field from a `Guardian`.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Classroom_userprofiles_guardians_getHandler(cfg),
	}
}
